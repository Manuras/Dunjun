...
namespace Input
{
	...
	void setCursorMode(CursorMode mode)
	{
		if (mode == CursorMode::Normal)
			glfwSetInputMode(
			    Game::getGlfwWindow(), GLFW_CURSOR, GLFW_CURSOR_NORMAL);
		if (mode == CursorMode::Hidden)
			glfwSetInputMode(
			    Game::getGlfwWindow(), GLFW_CURSOR, GLFW_CURSOR_HIDDEN);
		if (mode == CursorMode::Disabled)
			glfwSetInputMode(
			    Game::getGlfwWindow(), GLFW_CURSOR, GLFW_CURSOR_DISABLED);
	}
	...
	{
		glfwSetInputMode(
		    Game::getGlfwWindow(), GLFW_STICKY_MOUSE_BUTTONS, sticky);
	}
...
// 		break;
	...
	GamepadAxes getGamepadAxes(GamepadId gamepadId)
	{
		...
		axes.leftTrigger =
		    g_gamepadStates[gamepadId].Gamepad.bLeftTrigger / 255.0f;
		axes.rightTrigger =
